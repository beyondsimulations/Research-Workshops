...
A computational study must be reproducible. Report the hardware (CPU, RAM), the software (solver name and version, programming language), where the test data comes from or how you generated it, and what baselines you compare against (a benchmark, exact solution, or published results).

Report both solution quality (optimality gap, objective value) and computation time. Discuss how performance changes across instance sizes rather than just listing individual numbers.
...
: Chart type selection guide {#tbl-chart-types}

### Figure design

Label every axis with units. Use consistent colors across all figures: if Method A is blue in Figure 1, keep it blue everywhere. Remove gridlines, background shading, 3D effects, and decorative elements that don't encode data.

Make figures readable in grayscale (not everyone prints in color) by using line styles or markers in addition to colors. Avoid red-green combinations; the *viridis* and *cividis* palettes are safe for colorblind readers.

For computational studies, plotting the optimality gap or computation time on a log scale often reveals patterns that are invisible on a linear scale.

### Table design

Use booktabs style: horizontal rules only (`\toprule`, `\midrule`, `\bottomrule`), no vertical lines. Right-align numeric columns, left-align text columns. Put units in column headers, not in every cell. Keep tables compact and remove unnecessary columns. Every table needs a caption and must be referenced in the text.
...
Connect your results to the research question you posed in the introduction. Do your findings answer it? If only partially, say so explicitly. Compare with existing literature: are your results consistent with prior work? Where do they differ, and why?

Address limitations honestly. Every study has them. Discussing how limitations affect the interpretation of your results shows that you understand your own work. Hiding them does not.

If something did not work as expected, analyze why. Unexpected results are not a sign of failure. Ignoring them is.

Distinguish observation from interpretation. "The heuristic found solutions within 5% of the optimum for all small instances" is an observation. "This suggests the heuristic is suitable for small-scale real-world applications" is an interpretation. Don't conflate the two.
...
Separate summary from outlook. First state what you did and found, then suggest directions for future research.

Don't introduce new results here. The conclusion synthesizes; it does not analyze. Future research directions should be concrete: not "further research is needed" but "extending the model to include time windows would allow applications in last-mile delivery." Don't overclaim: "suggests" rather than "proves," "indicates" rather than "demonstrates," unless you provide formal proof.
...
Choose exact terms over vague ones. "Several studies" is better than "a lot of studies." A specific year is better than "recently." Avoid value judgments ("this excellent method") and emotional language ("unfortunately"). Let the evidence speak for itself.

Use the same term for the same concept throughout your paper. If you call it "optimality gap" on page three, don't switch to "solution quality deviation" on page eight. Readers will wonder whether you mean something different.

Use technical terms correctly and define them on first use if they are not standard in the field. Avoid filler words: "basically," "actually," "of course," "it is important to note that." These add length without adding meaning. Avoid discriminatory language and use inclusive formulations.
...
Read your paper aloud before submission. Listen for awkward phrasing, overly long sentences, and words that can be removed without changing the meaning. If a sentence spans more than three lines, split it. Clear writing usually signals clear thinking. The reverse is also true.
...
Plagiarism is not the only integrity issue. Scientific misconduct includes:

- **Data fabrication**: inventing data or results that were never observed. This is worse than reporting negative results. Negative results are a valid contribution; fabricated results are fraud.
- **Selective reporting**: only presenting the runs where your method worked well, omitting the ones where it didn't. Report all results, including failures.
- **Cherry-picking**: choosing metrics, instances, or parameter settings that make your approach look better than it is. If you tried 20 configurations and only report the best one, say so.
- **Misrepresenting sources**: citing a paper for a claim it doesn't actually make, or citing it without having read it.

None of these are worth the risk. Reviewers, supervisors, and examiners notice inconsistencies. If your method doesn't outperform the baseline, that is a valid finding worth discussing.

## Using AI tools

AI tools (ChatGPT, Claude, GitHub Copilot, Grammarly, and similar) may be used during your work. They can be helpful for brainstorming, debugging code, checking grammar, or exploring ideas. But there are rules.

You must declare all AI tool usage in the AI tools section of your paper. The templates include this section; see the [Templates](templates.qmd) page for how to fill it in. Undeclared use of AI-generated text is treated the same as any other uncited source.

More importantly: you are responsible for everything in your paper. If an AI tool produces an incorrect claim, a wrong citation, or a flawed argument and you include it in your work, that is your mistake, not the tool's. AI tools hallucinate references, invent plausible-sounding results, and confidently state things that are wrong. Every piece of AI output must be verified against the actual sources before you use it.

:::{.callout-important}
The rule is simple: if you cannot explain and defend every sentence in your paper during a discussion with your supervisor, it should not be in there, regardless of who or what wrote it.
:::
