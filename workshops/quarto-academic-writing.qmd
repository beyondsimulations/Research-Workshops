...
:::{.callout-tip}
Use a `.csl` file (e.g., `apa.csl`) to control citation and bibliography style. Set `csl: apa.csl` in your YAML frontmatter.
:::
...
- **Correct margins and formatting for UHH** -- page dimensions, font sizes, and spacing are pre-configured to meet the university's submission requirements, so you do not need to adjust them yourself.
- **Table of contents, list of figures, list of tables** -- these are generated automatically from your headings, figure captions, and table captions, keeping your document navigable as it grows.
- **Citation setup with APA style** -- the CSL file and bibliography path are already configured, meaning you can start citing sources with `[@key]` right away.
- **Example content showing all features** -- the template is pre-filled with sample sections, figures, tables, and equations that demonstrate correct syntax you can adapt for your own writing.
- **Declaration of authorship** -- the required signed statement is included at the end of the document, formatted according to university guidelines.
...
# Introduction
Your content here with [@citations] ...
